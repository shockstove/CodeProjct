...
void recvFile(SOCKET client,UDP_packet& pk,SOCKADDR_IN s,SOCKADDR_IN t){
    ...
    while(1){
        ...
        if(receive.checkChecksum()&&receive.flags&FLAG_USE){
            ...
            // }
            // else 
            if(receive.Seq > base){//大于窗口左侧
                RecvList[receive.Seq]=receive;
                recvFlag[receive.Seq]=1;
                if(rightestAck<receive.Seq){
                    rightestAck=receive.Seq;
                }
                send.setAck(receive.Seq);
                send.calChecksum();
                sendto(client,(char*)&send,sizeof(send),0,(SOCKADDR*)&t,sizeof(SOCKADDR));
        ...
        for(int i=base+1,right=base+WindowSize;i<=right;i++){
            ...
            // if(recvFlag[0]==0)break;
            // if(flag){
            //     if(recvFlag[i]){
            //         RecvList[exceptedSeqNum]=recvBuf[i];
            //         // memcpy(&recvFlag,&(recvFlag[1]),WindowSize-1);
            //         // recvFlag[WindowSize-1]=0;
            //         if(i == WindowSize-1){
            //             memset(&recvFlag,0,WindowSize);
            //         }
            //         if(recvBuf[i].flags&FLAG_END){
            //             length = recvBuf[i].length;
            //             cout<<"[Info]该数据包为最后一个包"<<endl;
            //             cout<<"[Info]--------文件接受完成--------"<<endl;
            //             goto FINISH;
            //         }
            ...
            //         flag = 0;
            //         tempPosi=i;
            //         recvBuf[i-tempPosi]=recvBuf[i];
            //         recvFlag[i-tempPosi]=recvFlag[i];
            //         recvFlag[i]=0;
            //     }
            // }
            // else{
